...
| `architecture/city/*` (buildings, road, sidewalk, cars, tree, bench, lamp) | 10 | **Done (decorative scope)** — a small baked street scene (`CityBackdrop`) visible through the office windows, varying by real-estate tier; `RealEstateManager` deliberately stays a management screen, no explorable spatial city (that would be a separate, much larger undertaking — see "City layer" below) |
...
- **A fully explorable, spatial city** (as opposed to the decorative backdrop above) — `RealEstateManager` stays a menu/data-driven management screen; turning real estate into literal navigable city geography is a product decision (new scene type, new navigation, new scope for what "moving offices" means), not an asset-integration task.
