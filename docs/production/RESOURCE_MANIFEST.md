...
## 3D models (38) — `res://assets/models/{characters,furniture,computers,datacenter,props}/*.glb`
See `docs/legal/ASSET_PROVENANCE.md`'s "3D asset pack integration" section for the full per-asset breakdown of wired vs. imported-only. Wired into real game systems: 6 character models (`StaffRoleCatalog.character_model` → `StaffAgent._build_visual()`, replacing the old P39/P40 procedural capsule body) and 2 buildable models (`BuildableCatalog`'s `model` field → `BuildController._make_real_mesh()`, replacing the procedural box for `desk` and `server_rack` specifically — `safety_lab` has no matching pack asset and correctly keeps its procedural box). `DataValidator` confirms every `character_model`/`model` path that's actually referenced resolves to a real file. All 38 (including the 30 not yet wired to a system) are validated and viewable via `scenes/dev/asset_gallery.tscn`. The pack's authored orientation is Z-up in local mesh space, not Godot's Y-up — confirmed by rendering one and looking at the pixels before assuming — so every loader applies a `-90°` X correction; see `StaffAgent`'s class-level doc comment for the full reasoning, including why the correction has to be on a dedicated wrapper node and not the agent itself.
