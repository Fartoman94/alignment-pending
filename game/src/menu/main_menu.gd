...
func _ready() -> void:
    _continue_button.disabled = not FileAccess.file_exists(SaveManager.SAVE_PATH)
...
func _on_continue_pressed() -> void:
    var err: Error = SaveManager.load_game()
    if err != OK:
        push_warning("MainMenu: continue failed to load save (error %s)" % err)
    await SceneRouter.go_to("res://scenes/campaign.tscn")
